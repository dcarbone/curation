...
class ValidationMainTest(TestCase):
    """
    _DT_FMT is the RFC3339 date format that gcloud timestamps are in because apparently
    the pythonic way is to have everybody define this themselves...
    """
    _DT_FMT = '%Y-%m-%dT%H:%M:%S.%fZ'

    def _build_mock_file_created(self, valid: bool):
        """
        ...
        if valid:
            out = now - datetime.timedelta(days=randint(0, 30))
        else:
            out = now - datetime.timedelta(days=randint(31, 9001))
        return out.strftime(self._DT_FMT)

    def _build_mock_file_updated(self, valid: bool):
        """
    ...
    def _build_mock_required_file_list(self,
        ...
        for req in common.AOU_REQUIRED_FILES:
            out.append(
                self._build_mock_file(filename=req,
                                      directory=directory,
    ...
    def test_retention_checks_list_submitted_bucket_items(self):
        outside_retention = datetime.datetime.today() - datetime.timedelta(
            days=29)
        outside_retention_str = outside_retention.strftime(self._DT_FMT)
        bucket_items = [{
    ...
    def test_folder_list(self):
        now = datetime.datetime.now()
        t0 = (now - datetime.timedelta(days=3)).strftime(self._DT_FMT)
        t1 = (now - datetime.timedelta(days=2)).strftime(self._DT_FMT)
        t2 = (now - datetime.timedelta(days=1)).strftime(self._DT_FMT)
        t3 = (now - datetime.timedelta(hours=1)).strftime(self._DT_FMT)
        expected = 't2/'
        bucket_items = list()
        for i in range(3):
            bucket_items.extend(
                self._build_mock_required_file_list(directory=f't{i}',
